...
struct qaFT0TrgBased {
  ...
  // Caution required if names are changed. See FillConditionhistos macro below (+ post processing scripts)
  ...
  void init(InitContext&)
  {
    ...
    const AxisSpec axisMultT0A{5000, 0., isLowFlux ? 10000. : 200000., "T0A multiplicity (# ADC channels)"};
    const AxisSpec axisMultT0C{1000, 0., isLowFlux ? 2000. : 80000., "T0C multiplicity (# ADC channels)"};
    const AxisSpec axisMultT0AC{12000, 0., isLowFlux ? 12000. : 240000., "T0AC multiplicity (# ADC channels)"};

    const AxisSpec axisMultV0A{2000, 0., isLowFlux ? 40000. : 200000., "V0A multiplicity"};

    const AxisSpec axisMultZNA{isLowFlux ? 500 : 1500, 0., isLowFlux ? 50. : 300., "E_{ZNA} (TeV)"};

    const AxisSpec axisMultZNC{isLowFlux ? 500 : 1500, 0., isLowFlux ? 50. : 300., "E_{ZNC} (TeV)"};

    const AxisSpec axisNcontrib{isLowFlux ? 150 : 4500, 0.,
                                isLowFlux ? 150. : 4500., "# contributors"};

    const AxisSpec axisCounter{2, 0.5, 1.5, ""}; // should be deleted as soon as BC task rewritten

    const AxisSpec axisEvSelStats{kNConditions, 0, kNConditions};

    // Lambda that creates one histogram for 'observable' per condition, excluding kAll condition
    bool isBCs = 0;

    auto const makeConditionHistos = [&](bool isBCs, eObservables observable, HistType histType, const std::vector<AxisSpec>& axis) {
      for (int iCondition = 1; iCondition < kNConditions; iCondition++) {
        ...
        const std::string histoName = prefix + mapObservableNames[observable] + std::string("/") + condition;
    ...
    if (doprocessCollisions) {
      ...
      makeConditionHistos(isBCs, kMultT0A_V0A, kTH2F, {axisMultT0A, axisMultV0A});

      makeConditionHistos(isBCs, kMultT0A_ZNA, kTH2F, {axisMultT0A, axisMultZNA});
      makeConditionHistos(isBCs, kMultT0C_ZNC, kTH2F, {axisMultT0C, axisMultZNC});

      makeConditionHistos(isBCs, kT0vertex, kTH1F, {axisVertex});

      histos.add("Colls/hPV/All", "PV;primary vertex (cm);counts", kTH1F, {axisVertex});
      ...
      histos.add("Colls/hContrib/All", ";(# contrubutors);counts", kTH1F, {axisNcontrib});
      makeConditionHistos(isBCs, kContrib, kTH1F, {axisNcontrib});

      // Histogram for storing event selection statistics
      auto hc = histos.add<TH1>("Colls/EventSelectionStats", "Event selection statistics", kTH1F, {axisEvSelStats});

      for (int iCondition = 0; iCondition < kNConditions; iCondition++) {
        hc->GetXaxis()->SetBinLabel(iCondition + 1, mapConditionNames[static_cast<eConditions>(iCondition)]);
      }
    }
    ...
    if (doprocessBCs) {
      ...
      histos.add("BCs/hMultZNA_ZNC/All", ";(# contrubutors);counts", kTH2F, {axisMultZNA, axisMultZNC});    // just to check ZDC
      histos.add("BCs/hMultZNA_ZNC_EM/All", ";(# contrubutors);counts", kTH2F, {axisMultZNA, axisMultZNC}); // just to check ZDC

      // Histogram for storing event selection statistics
      auto hbc = histos.add<TH1>("BCs/EventSelectionStats", "Event selection statistics", kTH1F, {axisEvSelStats});

      for (int iCondition = 0; iCondition < kNConditions; iCondition++) {
        hbc->GetXaxis()->SetBinLabel(iCondition + 1, mapConditionNames[static_cast<eConditions>(iCondition)]);
      }
    }
  }
  ...
  void
    processCollisions(soa::Join<aod::Collisions, aod::EvSels, aod::Mults,
                      ...
                      aod::FT0s const& ft0s, aod::FV0As const& fv0s, aod::Zdcs const& zdcs)
  {
    ...
    if (hasFT0) {
      ...
      // Macro for filling histograms for 'observable' based on conditions for CollisionsTask
...
#undef FillConditionHistos
  ...
  PROCESS_SWITCH(qaFT0TrgBased, processCollisions, "per-collision analysis", true);
                  ...
                  aod::FT0s const&, aod::Zdcs const& zdcs)
  {
    ...
    if (hasFT0) {
      ...
      // Macro for filling histograms for 'observable' based on conditions for BCsTask
...
{
  return WorkflowSpec{adaptAnalysisTask<qaFT0TrgBased>(cfgc, TaskName{"ft0-qa-trg-based"})};
}
