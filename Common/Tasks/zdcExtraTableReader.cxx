...
struct ZdcExtraTableReader {
  ...
  Configurable<int> Nbins5D{"Nbins5D", 4, "Bins in each dimension for 5D histograms"};
  ...
  void initHistos(const int& mRunNumber)
  {
    ...
    if (gEventCounter.find(mRunNumber) == gEventCounter.end()) {
      ...
      const AxisSpec axisCent5D = {Nbins5D, CentMin, CentMax, "Centrality (\%)"};
      const AxisSpec axisVx5D = {Nbins5D, VxMin, VxMax, "V_{x} (cm)"};
      const AxisSpec axisVy5D = {Nbins5D, VyMin, VyMax, "V_{y} (cm)"};
      const AxisSpec axisVz5D = {Nbins5D, VzMin, VzMax, "V_{z} (cm)"};
